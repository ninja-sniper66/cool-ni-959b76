...
async function generateModels() {
	...
	// models.dev sometimes renames Gemma 4 IDs; keep legacy IDs for compatibility.
	if (!allModels.some((m) => m.provider === "google" && m.id === "gemma-4-26b-it")) {
		allModels.push({
			id: "gemma-4-26b-it",
			name: "Gemma 4 26B",
			api: "google-generative-ai",
	...
	const ensureOpenRouterModel = (model: Model<"openai-completions">): void => {
		if (!allModels.some((m) => m.provider === "openrouter" && m.id === model.id)) {
			allModels.push(model);
		}
	};
	ensureOpenRouterModel({
